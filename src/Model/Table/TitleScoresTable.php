...
class TitleScoresTable extends AppTable
{
    ...
    public function findSummaryScores(): Query
    {
        ...
        $summary = $this->TitleScoreDetails->find()
            ->whereInList('division', ['勝', '敗'])
            ->select([
                'title_score_id' => 'title_score_id',
                'player1_id' => $query->func()->min('player_id'),
                'player2_id' => $query->func()->max('player_id'),
                'winner_player_id' => $query->expr("MAX(CASE WHEN division = '勝' THEN player_id END)"),
            ])
